...
void multiplyPolynomialsParallelNaive(std::vector<int> *a, std::vector<int> *b, std::vector<int> *result, int threadNum) {
...
Polynomial naiveParallelMultiplication(Polynomial A, Polynomial B, int numThreads, double &execTime) {
	auto a = A.getCoefficients();
	auto b = B.getCoefficients();
	auto result = std::vector<int>(a.size() + b.size() - 1, 0);
	auto start = std::chrono::steady_clock::now();
	multiplyPolynomialsParallelNaive(&a, &b, &result, numThreads);
...
Polynomial karatsubaSequentialMultiplication(Polynomial A, Polynomial B, double &execTime) {
	auto start = std::chrono::steady_clock::now();
	auto res = karatsubaSequential(A.getCoefficients(), B.getCoefficients());
	auto stop = std::chrono::steady_clock::now();
	auto diff = stop - start;
	auto time = std::chrono::duration<double, std::milli>(diff).count();
	time = time * 0.001;
...
/** Multiplying two numbers using the karatsuba approach. 
The method calls a auxiliary method with more parameters,then copies the result.
 */
...
Polynomial karatsubaParallelMultiplication(Polynomial A, Polynomial B, int numThreads, double &execTime) {
	auto result = std::vector<int>(A.getCoefficients().size() * 2, 0);
	auto a = A.getCoefficients();
	auto b = B.getCoefficients();
